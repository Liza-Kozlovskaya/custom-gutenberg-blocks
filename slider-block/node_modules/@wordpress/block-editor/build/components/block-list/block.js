...
function BlockListBlock(_ref2) {
  ...
  var isTypingWithinBlock = (0, _data.useSelect)(function (select) {
    var _select = select(_store.store),
        isTyping = _select.isTyping,
        hasSelectedInnerBlock = _select.hasSelectedInnerBlock;

    return (// We only care about this prop when the block is selected
      // Thus to avoid unnecessary rerenders we avoid updating the
      // prop if the block is not selected.
      (isSelected || hasSelectedInnerBlock(clientId, true)) && isTyping()
    );
  }, [clientId, isSelected]); // We wrap the BlockEdit component in a div that hides it when editing in
  ...
  var lightBlockWrapper = blockType.apiVersion > 1 || (0, _blocks.hasBlockSupport)(blockType, 'lightBlockWrapper', false); // Determine whether the block has props to apply to the wrapper.
...
var applyWithDispatch = (0, _data.withDispatch)(function (dispatch, ownProps, _ref5) {
  ...
  return {
    ...
    onMerge: function onMerge(forward) {
      ...
      var _select4 = select(_store.store),
